...
2. Implement the foundation for multiple layers with export functionality, allowing different layers to be saved/exported.
...
.grid-cell:hover {
  outline: 1px dashed red; /* Temporary highlight */
}
...
const exportAllLayersToSVG = () => {
  ...
  layers.forEach((layer) => {
    Object.keys(layer.cellColors).forEach((key) => {
      ...
      const x = (index % gridSize) * (resolution / gridSize);
      const y = Math.floor(index / gridSize) * (resolution / gridSize);
