...
2. Introduce logic to resize while keeping the art proportional.
...
const applyBrush = (index, size) => {
  ...
  for (let i = -size; i <= size; i++) {
    for (let j = -size; j <= size; j++) {
      const brushIndex = (y + i) * gridSize + (x + j);
      if (brushIndex >= 0 && brushIndex < gridSize * gridSize) {
...
2. SVG-to-PNG/JPG conversion on the server.

3. API for file uploads/downloads.
...
2. Add SVG-to-PNG/JPG conversion functionality using a back-end service.
...
3. Test SVG-to-PNG/JPG Conversion:
...
## Step 3: Define the Project Schema
...
## Step 2: Integrate Project Save and Load
...
## Step 3: Integrate Export Options
...
## Step 4: Test Front-End and Back-End Integration
