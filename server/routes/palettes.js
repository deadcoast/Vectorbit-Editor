...
router.put('/:id/tags', async (req, res) => {
  ...
  try {
    const updatedPalette = await Palette.findByIdAndUpdate(req.params.id, { tags }, { new: true });
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const updatedPalette = await Palette.findByIdAndUpdate(req.params.id, updateData, {
      new: true,
    });
    if (!updatedPalette) {
      return res.status(404).json({ error: 'Palette not found' });
