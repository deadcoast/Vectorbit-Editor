...
const optimizeCells = (gridSize, layerGridData) => {
  ...
  const processed = Array(gridSize)
    .fill()
  ...
  for (let y = 0; y < gridSize; y++) {
    for (let x = 0; x < gridSize; x++) {
      ...
      let width = 1;
      while (x + width < gridSize && grid[y][x + width] === color && !processed[y][x + width]) {
        width++;
      }
      ...
      outerLoop: while (y + height < gridSize) {
        for (let dx = 0; dx < width; dx++) {
          if (grid[y + height][x + dx] !== color || processed[y + height][x + dx]) {
            break outerLoop;
          }
        }
        height++;
...
>`;
  ...
  const layerContent = visibleLayers
    .map(layer => {
