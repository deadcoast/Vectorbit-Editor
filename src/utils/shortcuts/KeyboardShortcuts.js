...
export const ShortcutsProvider = ({ children, initialShortcuts = {} }) => {
  ...
  // Register a shortcut
  const registerShortcut = useCallback((id, context, keys, description, action) => {
  ...
  const setContext = useCallback(contexts => {
    ...
    setActiveContexts(['global', ...contextsArray.filter(context => context !== 'global')]);
  }, []);
  ...
  useEffect(() => {
    const handleKeyDown = event => {
      ...
      for (const [_id, shortcut] of Object.entries(activeShortcuts)) {
        if (
          shortcut.keys.some(key => matchesKeyCombination(key, keyCombination)) &&
          shortcut.action
        ) {
...
ShortcutsProvider.propTypes = {
  ...
  initialShortcuts: PropTypes.object,
};
...
const matchesKeyCombination = (definedCombination, actualCombination) => {
  ...
  if (Array.isArray(parsedDefinedCombination) && Array.isArray(parsedActualCombination)) {
    return (
      parsedDefinedCombination.length === parsedActualCombination.length &&
      parsedDefinedCombination.every(key => parsedActualCombination.includes(key))
    );
  }
