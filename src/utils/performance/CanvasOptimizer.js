...
class CanvasOptimizer {
  ...
  isCellVisible(x, y, size) {
    ...
    return x + size >= minX && y + size >= minY && x <= maxX && y <= maxY;
  }
  ...
  cleanCache() {
    ...
    for (const [key, timestamp] of entries) {
      if (
        now - timestamp > this.config.cacheExpiryMs ||
  ...
  updateAdaptiveConfig() {
    ...
    if (avgFrameTime > 33) {
      ...
      this.adaptiveConfig.lodThresholds.high = Math.min(
        2.0,
        this.adaptiveConfig.lodThresholds.high + 0.1
      );
      this.adaptiveConfig.lodThresholds.medium = Math.min(
        1.0,
        this.adaptiveConfig.lodThresholds.medium + 0.05
      );
      this.adaptiveConfig.lodThresholds.low = Math.min(
        0.5,
        this.adaptiveConfig.lodThresholds.low + 0.05
      );
    }
