      ...
      const timeoutId = setTimeout(() => {
        if (resourceState.pending.has(id)) {
          ...
          if (attempts >= retryAttempts) {
            ...
            if (config.debug) {
              console.error(`[LazyLoader] Failed to load component after ${retryAttempts} attempts: ${id}`);
            }
            ...
            // Retry loading
            if (config.debug) {
              console.warn(`[LazyLoader] Retrying component load (${attempts + 1}/${retryAttempts}): ${id}`);
            }
...
const processPrefetchQueue = () => {
  ...
  const highAndMediumItems = resourceState.prefetchQueue.filter(
    item => item.priority === config.priority.HIGH || 
            item.priority === config.priority.MEDIUM
  );
  
  // Load each item
  highAndMediumItems.forEach(item => {
    // Remove from queue
    resourceState.prefetchQueue = resourceState.prefetchQueue.filter(
      i => i.id !== item.id
    );
...
export const preloadImage = (src, options = {}) => {
  const {
    id = `img_${src.replace(/[^a-zA-Z0-9]/g, '_')}`,
    priority = config.priority.MEDIUM
  } = options;
