...
import { createOffscreenCanvas } from './canvasUtils';
 ...
 */
export const applyBrushStroke = (strokeData) => {
  const {
    x,
    y,
    color,
...
const applyStampWithSpacing = (ctx, stamp, x, y, spacing, opacity, blendMode) => {
  ...
  const currentContent = ctx.getImageData(
    x - offsetX,
    y - offsetY,
    stampWidth,
    stampHeight
  ...
  for (let y = -radius; y <= radius; y++) {
    for (let x = -radius; x <= radius; x++) {
      ...
      if (dist <= radius && Math.random() < density) {
        ...
        ctx.fillRect(
          center + x - grainSize/2,
          center + y - grainSize/2,
          grainSize,
          grainSize
        );
      }
    ...
    for (let bx = 0; bx < width; bx += dotSize * 2) {
      ...
      for (let y = by; y < Math.min(by + dotSize * 2, width); y++) {
        for (let x = bx; x < Math.min(bx + dotSize * 2, width); x++) {
          const index = (y * width + x) * 4;
          const brightness = (tempPixels[index] + tempPixels[index + 1] + tempPixels[index + 2]) / 3;
          total += brightness;
          count++;
        }
      }
