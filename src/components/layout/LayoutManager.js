...
const DEFAULT_PANEL_POSITIONS = {
  ...
  canvas: { x: 300, y: 60, width: 'calc(100% - 500px)', height: 'calc(100% - 60px)', isVisible: true },
  colors: { x: 'calc(100% - 200px)', y: 60, width: 200, height: 200, isVisible: true },
  history: { x: 'calc(100% - 200px)', y: 260, width: 200, height: 'calc(100% - 260px)', isVisible: true }
};
...
export const LayoutProvider = ({ children, initialLayout = {} }) => {
  ...
  const setPanelPosition = useCallback((panelId, position) => {
    ...
    saveLayoutToLocalStorage({ ...panelPositions, [panelId]: { ...panelPositions[panelId], ...position } });
  }, [panelPositions]);
  ...
  const saveCurrentLayout = useCallback((layoutName) => {
...
export const DraggablePanel = ({ 
  ...
  useEffect(() => {
    ...
    id, isDragging, isResizing, dragOffset, resizeStartPos, resizeStartDim, 
    position, minWidth, minHeight, setPanelPosition
  ]);
  ...
  const panelStyle = {
    ...
    width: isCollapsed ? 'auto' : (typeof position.width === 'string' ? position.width : `${position.width}px`),
    height: isCollapsed ? 'auto' : (typeof position.height === 'string' ? position.height : `${position.height}px`),
  };
  ...
  return (
    ...
    >
      {!hideHeader && (
        ...
        >
          {icon && <span className={`panel-icon icon-${icon}`}></span>}
...
export const PanelToggle = ({ panelId, tooltip }) => {
  ...
  return (
    ...
    >
      <span className={`icon-${panelDef.icon || panelId}`}></span>
