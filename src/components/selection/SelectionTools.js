...
const SelectionTools = ({ 
    ...
    switch (selectionMode) {
      ...
      case SELECTION_MODES.MAGIC_WAND:
        ...
        if (selectionStart.x >= 0 && selectionStart.y >= 0 && 
            selectionStart.x < gridSize && selectionStart.y < gridSize) {
          
          const targetCell = layerData.find(
            cell => cell.layerId === activeLayer && 
        ...
        if (selectionStart.x >= 0 && selectionStart.y >= 0 && 
          ...
          if (targetCell) {
            ...
            layerData.forEach(cell => {
              if (cell.layerId === activeLayer && isColorSimilar(cell.color, targetColor, tolerance)) {
                cells.push(cell);
              }
            });
          }
    ...
    switch (transformType) {
      ...
      case TRANSFORM_TYPES.SCALE:
        ...
        const initialDistance = Math.sqrt(
          Math.pow(selectionStart.x - originX, 2) + 
          Math.pow(selectionStart.y - originY, 2)
        );
  ...
  }, [isSelecting, selectionStart, selectionEnd, selectedCells, selectionMode, isTransforming, transformParams]);
          ...
          <div className="transform-controls">
            ...
            {isTransforming && (
              <div className="transform-actions">
                <button onClick={applyTransform} className="apply-button">
                  Apply
                </button>
                <button onClick={cancelTransform} className="cancel-button">
                  Cancel
                </button>
              </div>
            )}
