...
describe("AdvancedColorWheel Component", () => {
  it("renders the color picker and add-to-palette button", () => {
    const { getByText } = render(
      <AdvancedColorWheel activeColor="#ffffff" setActiveColor={jest.fn()} onAddToPalette={jest.fn()} />
    );
  ...
  it("updates color on change", () => {
    ...
    const { getByRole } = render(
      <AdvancedColorWheel activeColor="#ffffff" setActiveColor={mockSetActiveColor} onAddToPalette={jest.fn()} />
    );
