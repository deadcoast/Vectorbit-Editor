...
const ColorManager = ({
  ...
  const allPalettes = useMemo(
    () => [...presets.palettes, ...customPalettes],
    [customPalettes]
  );

  const allGradients = useMemo(
    () => [...presets.gradients, ...customGradients],
    [customGradients]
  );
  ...
  return (
    <div className="color-manager">
      ...
      <button
        onClick={() =>
          handleAddPalette(prompt("Enter HEX colors separated by commas:").split(","))
        }
      >
        Add Custom Palette
      </button>
      ...
      <button onClick={() => handleGenerateComplementary("#ff7e5f")}>
        Generate Complementary
      </button>
      <button onClick={() => handleGenerateAnalogous("#6a11cb")}>
        Generate Analogous
      </button>
      <button
        onClick={() =>
          alert(handleAdjustBrightness("#ff7e5f", 20))
        }
      >
        Brighten Color
      </button>
      <button
        onClick={() =>
          alert(handleBlendColors("#ff7e5f", "#2575fc", 0.5))
        }
      >
        Blend Colors
      </button>
