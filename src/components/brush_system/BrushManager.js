...
import {
  blendColors,
  generateGradientPalette as generateGradient,
...
const BrushManager = ({
   ...
   */
  const determineCellColor = useCallback(
    (x, y) => {
   ...
   */
  const applyCoordinateSymmetry = useCallback(
    (x, y) => {
      const points = [{ x, y }];
      const centerX = gridSize / 2;
      const centerY = gridSize / 2;
   ...
   */
  const calculatePressureSize = useCallback(
    basePressure => {
      const avgPressure =
        pressurePoints.current.reduce((sum, p) => sum + p, 0) /
  ...
  const applyStroke = useCallback(
    (x, y, currentPressure = pressure) => {
      ...
      const points =
        brushSmoothing > 0 && strokeHistory.current.length >= 4
      ...
      symmetricPoints.forEach(point => {
        ...
        applyBrush(strokeData.x, strokeData.y);
        onStroke?.(strokeData);
      });
    },
    [
    ...
    return (
      <div className="brush-controls">
        ...
        {brushType === BRUSH_TYPES.PATTERNED && (
          <div className="pattern-controls">{/* Existing pattern controls */}</div>
        )}

        {/* Gradient Controls */}
        {brushType === BRUSH_TYPES.GRADIENT && (
          <div className="gradient-controls">{/* Existing gradient controls */}</div>
        )}
