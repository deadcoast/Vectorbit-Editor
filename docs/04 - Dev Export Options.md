...
The export functionality will enable saving the current grid as .svg, .png, or .jpg files. For now,
we’ll create a working placeholder that:
...
Update the App component to manage cell states globally, so we can reuse the state for export
operations.
...
We will update the grid controls to include buttons for .png and .jpg export, although they will
currently only log a message.
...
1. Verify functionality: • Clicking Export to SVG downloads the current grid as an .svg file. •
   Clicking Export to PNG or Export to JPG logs placeholders in the console.

## Step 1: Add a Canvas Element for Rendering

To implement full functionality for exporting to .png and .jpg, we’ll use the Canvas API to render
the grid and convert it into rasterized image formats. Here’s how we’ll integrate this functionality
step-by-step:
